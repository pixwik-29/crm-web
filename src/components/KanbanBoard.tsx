...
import { Phone, MessageSquare, Tag, Award, User, Flame } from 'lucide-react';
...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({ leads, profiles, onSelectLead }) => {
  const { updateLead, currentUser, settings } = useData();
  const stages = [...(settings.pipeline_stages || [])].sort((a, b) => a.order - b.order);
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${stage.color}`}>
                  {stage.name}
                </span>
