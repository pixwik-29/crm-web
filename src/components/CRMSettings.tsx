...
export const CRMSettings: React.FC = () => {
  ...
  const handleToggleSubscription = async (page: any) => {
    setIsSubscribingId(page.id);
    const isCurrentSubscribed = !!page.is_subscribed;
    try {
      if (isCurrentSubscribed) {
        // Unsubscribe
        const res = await fetch(`https://graph.facebook.com/v19.0/${page.id}/subscribed_apps?access_token=${page.access_token}`, {
          method: 'DELETE'
        });
        const data = await res.json();
        if (data.success) {
          const updatedPages = connectedPages.map((p: any) => 
            p.id === page.id ? { ...p, is_subscribed: false } : p
          );
          await updateSettings({ fb_pages: updatedPages });
          alert(`Successfully unsubscribed ${page.name} from Lead Webhooks.`);
        } else {
          alert(`Meta error: ${data.error?.message || 'Failed to unsubscribe'}`);
        }
      } else {
        // Subscribe
        const res = await fetch(`https://graph.facebook.com/v19.0/${page.id}/subscribed_apps?subscribed_fields=leadgen&access_token=${page.access_token}`, {
          method: 'POST'
        });
        const data = await res.json();
        if (data.success) {
          const updatedPages = connectedPages.map((p: any) => 
            p.id === page.id ? { ...p, is_subscribed: true } : p
          );
          await updateSettings({ fb_pages: updatedPages });
          alert(`Success! ${page.name} is now linked to the CRM. Lead Ads will now flow in automatically.`);
        } else {
          alert(`Meta error: ${data.error?.message || 'Failed to subscribe'}`);
        }
      }
    } catch (err: any) {
            ...
            <div className={`rounded-2xl border p-4 space-y-4 ${isFbConnected ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800/40' : 'bg-slate-50 dark:bg-zinc-900/40 border-slate-200 dark:border-zinc-800'}`}>
              <div className="flex items-center justify-between">
                ...
                {isFbConnected ? (
                  ...
                  <a href={`/api/fb-oauth?tenant_id=${tenantId}`} className="px-3 py-1.5 rounded-xl text-[10px] font-bold text-white transition-all flex items-center gap-1.5 bg-[#1877F2] hover:bg-[#166fe5] cursor-pointer hover:scale-[1.02] active:scale-[0.98]">
              ...
              {!isFbConnected && (
                <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-3.5 space-y-2 text-[10px] text-amber-600 dark:text-amber-400 leading-relaxed animate-fade-in">
                  ...
                  <ul className="list-decimal pl-4 space-y-1.5 font-medium">
                    ...
                    <li>Ensure your Meta App status is switched to <strong>Live Mode</strong> and that <code>public_profile</code>, <code>pages_show_list</code>, <code>pages_read_engagement</code>, and <code>leads_retrieval</code> are approved with Advanced Access.</li>
                  </ul>
                </div>
              )}
               ...
               {isFbConnected && connectedPages.length > 0 && (
                <div className="pt-3 border-t border-slate-200 dark:border-zinc-800/60 space-y-2">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <p className="text-[9px] text-slate-400 font-semibold leading-relaxed">Subscribe a page to connect its Lead Ads forms to this CRM workspace.</p>
                  </div>
                  <div className="grid grid-cols-1 gap-2">
                    {connectedPages.map((page: any) => {
                      const isSubscribed = !!page.is_subscribed;
                      const isLoading = isSubscribingId === page.id;
                      return (
                        <div key={page.id} className="bg-white/80 dark:bg-black/60 border border-slate-200 dark:border-zinc-900 px-4 py-3 rounded-2xl flex flex-wrap items-center justify-between text-[11px] text-slate-800 dark:text-slate-200 gap-3">
                          <div className="flex flex-col gap-0.5">
                            <span className="font-bold text-xs">{page.name}</span>
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-[8.5px] text-slate-400 font-mono">ID: {page.id}</span>
                              <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[8.5px] font-bold uppercase ${
                                isSubscribed 
                                  ...
                                  : 'bg-slate-100 text-slate-500 dark:bg-zinc-900 dark:text-slate-400 border border-slate-200 dark:border-zinc-800'
                              }`}>
                                <span className={`w-1 h-1 rounded-full ${isSubscribed ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                                {isSubscribed ? 'Linked (Webhook Active)' : 'Not Linked'}
                              </span>
                            </div>
                          </div>
                          ...
                          <button
                            type="button"
                            disabled={isLoading}
                            onClick={() => handleToggleSubscription(page)}
                            className={`px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                              isSubscribed
                                ? 'bg-slate-100 hover:bg-slate-250 dark:bg-zinc-900 dark:hover:bg-zinc-850 text-slate-700 dark:text-slate-300'
                                : 'bg-[#1877F2] hover:bg-[#166fe5] text-white shadow shadow-blue-500/10 hover:scale-[1.02] active:scale-[0.98]'
                            }`}
                          >
                            {isLoading ? (
                              <svg className="animate-spin h-3.5 w-3.5 text-current" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>
                            ) : isSubscribed ? (
                              'Disconnect Webhook'
                            ) : (
                              'Link to CRM'
