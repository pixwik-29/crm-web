  ...
  const addLead = async (leadData: Omit<Lead, 'id' | 'created_at' | 'updated_at'>): Promise<Lead> => {
    ...
    if (isSupabaseConfigured && supabase) {
      ...
      if (data) {
        ...
        // Send push notification via server-side API (uses service role key to bypass RLS)
        fetch('/api/notify/new-lead', {
          ...
          body: JSON.stringify({
            leadId: data.id,
            leadName: data.name,
            neetMarks: data.neet_marks,
            leadSource: data.lead_source,
            excludeEmail: currentUser?.email
          })
        }).catch(err => console.error('[AddLead Push] Failed to call notify endpoint:', err));
      }
      return data as Lead;
    } else {
